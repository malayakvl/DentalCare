...
class OutgoingInvoiceController extends Controller
{
    ...
    public function create(Request $request): Response {
        if ($request->user()->can('invoice-outgoing-create')) {
            ...
            $customerData = DB::table('users')
                ...
                ->where('clinic_id', $clinicData->id)
                ->orderBy('name')->get();
            return Inertia::render('InvoiceOutgoing/Create', [
    ...
    public function edit(Request $request, $id) {
        if ($request->user()->can('invoice-outgoing-edit')) {
            $clinicData = Clinic::where('user_id', '=', $request->user()->id)->first();
            $storeData = Store::where('clinic_id', $clinicData->id)->get();
            $statusData = InvoiceStatus::all();
            $typeData = InvoiceType::all();
            ...
            return Inertia::render('InvoiceOutgoing/Edit', [
                'clinicData' => $clinicData,
                'filialData' => $storeData,
                'formData' => $formData,
                'formRowData' => $rowData,
    ...
    public function show(Request $request, $id) {
        //
        if ($request->user()->can('invoice-outgoing-view')) {
