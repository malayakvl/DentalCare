...
import { addUploadedFile, removeUploadedFile } from '@/redux/clinics';
import { useTranslations } from 'next-intl';
import {useDispatch, useSelector} from 'react-redux';
import { removeProductFileAction } from '@/redux/clinics/actions';
import {uploadedFilesSelector} from "@/redux/clinics/selectors";
...
const InventoryPhotos: React.FC<any> = ({ productData, uploadedFiles, photos }) => {
    ...
    const addedZonePhotos = useSelector(uploadedFilesSelector);
    ...
    return (
        <>
            <h2 className="form-subtitle">{t('Clinic images')}</h2>
            <section className="drop-zone-container">
                ...
                <div></div>
                {(addedZonePhotos.length > 0 || photos.length > 0) && (
                    <aside>
                        <h4>{t('Uploaded Files')}</h4>
                        <ul>
                            {addedZonePhotos.map((_file: File) => (
