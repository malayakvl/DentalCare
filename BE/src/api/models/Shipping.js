...
class Shipping {
    ...
    async changeCustomerStatus(status, customerId, shippingId) {
        const client = await pool.connect();
        // console.log('=============')
        // console.log(status, shippingId);
        let query;
        try {
            console.log(`DELETE FROM data.customer_disabled_shipping WHERE user_id = ${customerId} AND shipping_id = ${shippingId}`);
            await client.query(`DELETE FROM data.customer_disabled_shipping WHERE user_id = ${customerId} AND shipping_id = ${shippingId}`);
            // query =
            //     `DELETE FROM data.customer_disabled_shipping WHERE user_id = ${customerId} AND shipping_id = ${shippingId}`;
            // console.log("Status", status);
            if (status) {
                console.log(`INSERT INTO data.customer_disabled_shipping (user_id, shipping_id) VALUES ('${customerId}', '${shippingId}')`);
                await client.query(`INSERT INTO data.customer_disabled_shipping (user_id, shipping_id) VALUES ('${customerId}', '${shippingId}')`);
                // query =
                //     `INSERT INTO data.customer_disabled_shipping (user_id, shipping_id) VALUES ('${customerId}', '${shippingId}')`;
            }
    ...
    async setThreshold(userId, data) {
        const client = await pool.connect();
        try {
            ...
            client.query(
                ...
                [userId, data.threshold]
            )
            await client.query(`UPDATE data.seller_settings SET free_shipping_from='${data.dateFrom}', free_shipping_to='${data.dateTo}' WHERE user_id=${userId}`);
    ...
    async setShippingPeriod(userId, dateFrom, dateTo) {
        const client = await pool.connect();
        try {
            await client.query(`UPDATE data.seller_settings SET free_shipping_from='${dateFrom}', free_shipping_to='${dateTo}' WHERE user_id=${userId}`);
    ...
    async clearShippingPeriod(userId) {
        const client = await pool.connect();
        try {
            await client.query(`UPDATE data.seller_settings SET free_shipping_from=null, free_shipping_to=null WHERE user_id=${userId}`);
            await client.query(`UPDATE data.free_order_threshold SET threshold=null WHERE user_id=${userId}`);
            // await client.query('UPDATE data.free_order_threshold WHERE user_id = $1', [userId]);
    ...
    async getShippingPeriod(userId) {
        const client = await pool.connect();
        try {
            await client.query(`UPDATE data.seller_settings SET free_shipping_from='${dateFrom}', free_shipping_to='${dateTo}' WHERE user_id=${userId}`);
    ...
    async fetchShippingPeriod(userId) {
        const client = await pool.connect();
        try {
            const res = await client.query(`SELECT free_shipping_from, free_shipping_to FROM data.seller_settings WHERE user_id=${userId}`);
